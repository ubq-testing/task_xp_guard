...
async function checkUserExperience(
    ...
    const tiers = Object.entries(xpTiers).reduce((acc, [key, value]) => {
        acc[key.toLowerCase()] = value;
        return acc;
    }, {} as Record<string, number>);
    const langCheck = await handleLabelChecks(context, token, labelFilters, tiers, user);
    const statCheck = await handleStatChecks(context, token, user, minCommitsThisYear, prs, issues, stars);

    return !!(statCheck && langCheck);
}

async function findAndRemoveAssignee(
    context: Context,
    user: string
) {
